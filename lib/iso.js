function Iso(x,y, container, options) {
  ...
  if (options) {
    ...
    if (options.shadow_offset) {
      this.shadow_offset = options.shadow_offset;
    } else {
      this.shadow_offset = options.shadow_offset = options.avatar_offset;
    }
  }
  ...
  this.place_avatar = function(avatar_index) {
    ...
    shadow_element.animate({
      left: node.left() + this.shadow_offset[0],
      top: node.top() + this.shadow_offset[1]
    }, 200);
  };
  ...
  this.render = function() {
    for (var i=0; i < this.avatars.length; i++) {
      ...
      shadow_element.css('left',this.shadow_offset[0]);
      shadow_element.css('top',this.shadow_offset[1]);
