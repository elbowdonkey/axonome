(function($) {
  $.fn.iso = function(options) {
    ...
    return this.each(function() {
      ...
      //if ($.fn.iso.avatars.length == 1) {
        $.fn.iso.avatar = $.fn.iso.avatars[0];
      //};
      
      render_tiles(this);
      render_avatars(this);
      attach_handlers(this);
    });
  };
  ...
  function attach_handlers(container) {
  ...
  function place_avatar(avatar_index) {
    ...
    if ($.fn.iso.defaults.animate == true) {
      avatar_element.animate({
    		left: node.left() + $.fn.iso.defaults.avatar_offset[0],
    		top: node.top() + $.fn.iso.defaults.avatar_offset[1]
    	}, $.fn.iso.defaults.speed);
  ...
  function follow_path() {
    var avatar = $.fn.iso.avatar;
    var avatar_element = $('#avatar_0');
    var node = $.fn.iso.avatar.node();
    
    if ($.fn.iso.defaults.animate == true) {
      avatar_element.animate({
    		left: node.left() + defaults.avatar_offset[0],
    		top: node.top() + defaults.avatar_offset[1]
    	}, $.fn.iso.defaults.speed, function() {
