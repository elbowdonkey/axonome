function A(x,y,grid) {
  ...
  this.add_node = function(x,y,node) {
    var node_name = x + "_" + y;
    this.master_list[node_name] = {"status": 0, "x": x, "y": y, "parent": undefined, "f": 0, "g": 0, "h": 0, "walkable": node.walkable, "node": node};
  }
  
  for (var i=0; i < this.grid.nodes.length; i++) {
    var x = this.grid.nodes[i].x;
    var y = this.grid.nodes[i].y;
    this.add_node(x,y,this.grid.nodes[i]); 
  };
  ...
  this.toggle_walkability = function(node) {
    // TODO: give Node sole dominion over modifying nodes
    ...
    node["walkable"] = node.node["walkable"];
  }
  ...
  this.find_path = function() {
    ...
    while (path_found == false) {
      ...
      this.path.push([current_node["x"], current_node["y"]]);
